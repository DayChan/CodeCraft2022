...
// 提交的时候要注释掉这个宏
#define DEBUG 
void handle_csv(std::ifstream& fp, std::unordered_map<std::string ,std::vector<std::string>>& output, std::vector<std::string>& columns) {
    std::string line;
    getline(fp,line); //列名
...
void handle_dm(std::unordered_map<std::string ,std::vector<std::string>>& data_dm, std::vector<std::string>& cols_client, std::vector<std::vector<int>>& data_dm_rowstore) {
...
// void handle_qos(std::unordered_map<std::string ,std::vector<std::string>>& data_qos, std::vector<std::string>& cols_qos, std::unordered_map<std::string, std::unordered_map<std::string, int>>& output, std::vector<std::string>& cols_edges) {
//     for(size_t i=0; i < data_qos["site_name"].size(); i++) {
//         for(size_t j=1; j<cols_qos.size();j++) {
//             output[data_qos["site_name"][i]][cols_qos[j]] = atoi(data_qos[cols_qos[j]][i].c_str());
//         }
//     }
//     cols_edges = data_qos["site_name"];
// }

void handle_qos_filter(std::unordered_map<std::string ,std::vector<std::string>>& data_qos, std::vector<std::string>& cols_client, std::vector<std::vector<int>>& output, std::vector<int>& edge_dist_num, std::vector<std::string>& cols_edges, int qos_constrain) {
    cols_edges = data_qos["site_name"];
...
void handle_sb(std::unordered_map<std::string ,std::vector<std::string>>& data_sb, std::unordered_map<std::string, int>& sb_map) {
...
void handle_contest_input(std::vector<std::string>& cols_client, std::vector<std::string>& cols_edges, std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map, std::vector<int>& edge_dist_num, std::unordered_map<std::string, int>& sb_map, int& qos_constrain) {
    #ifdef DEBUG
...
void contest_calculate_avg(std::vector<std::string>& client_names, std::vector<std::string>& edges_names, std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map, std::vector<int>& edge_dist_num,  std::unordered_map<std::string, int>& sb_map, int qos_constrain, std::vector<std::vector<std::vector<int>>>& res) {
    // //
...
void contest_calculate_95_attack1(std::vector<std::string>& client_names, std::vector<std::string>& edges_names, std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map, std::vector<int>& edge_dist_num,  std::unordered_map<std::string, int>& sb_map, int qos_constrain, std::vector<std::vector<std::vector<int>>>& res) {
    res.resize(data_dm_rowstore.size()); // 时间 客户节点序号 edge序号 分配数量
    int split_max = 1000;
...
void contest_calculate_95_attack2(std::vector<std::string>& client_names, std::vector<std::string>& edges_names, std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map, std::vector<int>& edge_dist_num,  std::unordered_map<std::string, int>& sb_map, int qos_constrain, std::vector<std::vector<std::vector<int>>>& res) {
    res.resize(data_dm_rowstore.size()); // 时间 客户节点序号 edge序号 分配数量
    int split_max = 1000;
    std::vector<int> edge_times(edges_names.size(), 0);
    int five_percent_time = data_dm_rowstore.size() * 0.05 - 1;
...
void contest_calculate_95_attack3(std::vector<std::string>& client_names, std::vector<std::string>& edges_names, std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map, std::vector<int>& edge_dist_num,  std::unordered_map<std::string, int>& sb_map, int qos_constrain, std::vector<std::vector<std::vector<int>>>& res) {
    res.resize(data_dm_rowstore.size()); // 时间 客户节点序号 edge序号 分配数量
    int split_max = 1000;
    std::vector<int> edge_times(edges_names.size(), 0);
    int five_percent_time = data_dm_rowstore.size() * 0.05 - 1;
...
void contest_calculate_95_attack4(std::vector<std::string>& client_names, std::vector<std::string>& edges_names, std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map, std::vector<int>& edge_dist_num,  std::unordered_map<std::string, int>& sb_map, int qos_constrain, std::vector<std::vector<std::vector<int>>>& res) {
    res.resize(data_dm_rowstore.size()); // 时间 客户节点序号 edge序号 分配数量
    std::vector<int> edge_times(edges_names.size(), 0);
    int five_percent_time = data_dm_rowstore.size() * 0.05 - 1;
...
void contest_calculate_95_with_maxrecord(std::vector<std::string>& client_names, std::vector<std::string>& edges_names, std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map, std::vector<int>& edge_dist_num,  std::unordered_map<std::string, int>& sb_map, int qos_constrain, std::vector<std::vector<std::vector<int>>>& res) {
    res.resize(data_dm_rowstore.size()); // 时间 客户节点序号 edge序号 分配数量
    std::vector<int> edge_times(edges_names.size(), 0);
    int five_percent_time = data_dm_rowstore.size() * 0.05 - 1;
    std::vector<bool> edge_exceed(edges_names.size(), false);
    ...
    for(size_t time=0; time < data_dm_rowstore.size(); time++) {
        ...
        for(size_t cli_idx=0; cli_idx < row.size(); cli_idx++) {
            ...
            for(size_t i=0; i < qos_map[cli_idx].size(); i++) {
                max_record_sort.push_back({edge_max_record[qos_map[cli_idx][i]], qos_map[cli_idx][i]});
            }
            std::sort(max_record_sort.begin(), max_record_sort.end(), std::greater<std::pair<int, int>>());
            ...
            for(size_t i=0; i < qos_map[cli_idx].size(); i++) {
                sb_rest_sort.push_back({sb_map_cp[edges_names[qos_map[cli_idx][i]]], qos_map[cli_idx][i]});
            }
            std::sort(sb_rest_sort.begin(), sb_rest_sort.end(), std::greater<std::pair<int, int>>());
...
void handle_output(std::vector<std::vector<std::vector<int>>>& res, std::vector<std::string>& client_names, std::vector<std::string>& edges_names) {
    #ifdef DEBUG
...
int main() {
    ...
    std::vector<std::vector<int>> data_dm_rowstore; // demand行存，外idx为时间排序，内idx根据客户节点vector排序
    std::vector<std::vector<int>> qos_map; // 根据qos_constrain过滤后客户节点可用的edges，vector序号对应着client_names里面的顺序，edges序号对应着edges_names里面的序号
    std::vector<int> edge_dist_num;        // 每个edge可以分发的节点数
    std::unordered_map<std::string, int> sb_map; // 带宽映射，根据edge name获得带宽
    int qos_constrain;
    
    handle_contest_input(client_names, edges_names, data_dm_rowstore, qos_map, edge_dist_num, sb_map, qos_constrain);
    // input test
    ...
    contest_calculate_95_with_maxrecord(client_names, edges_names, data_dm_rowstore, qos_map, edge_dist_num, sb_map, qos_constrain, res_for_print);
