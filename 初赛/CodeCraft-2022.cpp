        ...
        for (size_t st = 0; st < sort_time.size(); st++) {
            ...
            for (auto& cli : sort_cli_dm_in) {
                ...
                for (size_t i = 0; i < qos_map[cli_idx].size(); i++) {
                    sort_edge_avg_dist_highest_value.push_back(
                        {sb_map[edges_names[qos_map[cli_idx][i]]] /
                             (edge_dist_num[qos_map[cli_idx][i]] + 0.1),
                         qos_map[cli_idx][i]});
                }
...
void contest_calculate_95_with_maxrequire_sort16_adjust1(
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
    std::unordered_map<std::string, int>& sb_map, int qos_constrain,
    std::vector<std::vector<std::vector<int>>>& res, double threshold) {
    int split_max = 1000;
    ...
    for (int i = 0; i < edges_names.size() * threshold; i++) {
        int edge_idx = std::get<2>(sort_edge_dist_num[i]);
        // if (first_alloc[edge_idx]) continue;
        std::vector<std::pair<int, int>>& sort_require = edge_require_sort[edge_idx];
        auto& clis = edge_dist_clients[edge_idx];
...
void contest_calculate_95_with_maxrequire_sort16_adjust2(
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
    std::unordered_map<std::string, int>& sb_map, int qos_constrain,
...
void contest_calculate_95_with_maxrequire_sort17_adjust1(
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
    std::unordered_map<std::string, int>& sb_map, int qos_constrain,
...
void contest_calculate_95_with_maxrequire_sort17_adjust2(
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
    std::unordered_map<std::string, int>& sb_map, int qos_constrain,
...
void contest_calculate_95_with_maxrequire_sort18_adjust(
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
    std::unordered_map<std::string, int>& sb_map, int qos_constrain,
...
void contest_calculate_95_with_maxrequire_sort14_adjust(
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
    std::unordered_map<std::string, int>& sb_map, int qos_constrain,
...
void contest_calculate_95_with_maxrequire_sort19(
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
    std::unordered_map<std::string, int>& sb_map, int qos_constrain,
    ...
    std::vector<bool> first_alloc(edges_names.size());
    {
        ...
        std::vector<std::unordered_map<std::string, int>> sb_map_alltime(data_dm_rowstor1.size(),
                                                                         sb_map_copy);
        std::vector<double> clients_avg(client_names.size(), 0);
        for (int i = 0; i < data_dm_rowstor1.size(); i++) {
            std::vector<int>& row = data_dm_rowstor1[i];
            int sumrow = 0;
            for (int j = 0; j < client_names.size(); j++) {
        ...
        for (int i = 0; i < client_names.size(); i++) {
            clients_avg[i] /= data_dm_rowstor1.size();
        }
        ...
        for (int i = 0; i < edges_names.size(); i++) {
            ...
            while (five_count <= five_percent_time && require_idx < sort_require.size()) {
                ...
                std::vector<int>& row = data_dm_rowstor1[time];
                int dist_v_acc = 0;
                std::vector<std::pair<double, int>> sort_cli_dm(clis.size());
                for (int j = 0; j < clis.size(); j++) {
                    double cli_dm_diff = row[clis[j]] - clients_avg[clis[j]];
                    sort_cli_dm[j] = {cli_dm_diff, clis[j]};
                    // int cli_edge_nums = qos_map[clis[j]].size();
                    // sort_cli_dm[j] = {cli_edge_nums, clis[j]};
                }
                std::sort(sort_cli_dm.begin(), sort_cli_dm.end(),
                ...
                int lower_avg_loc = 0;
                for (int j = 0; j < sort_cli_dm.size(); j++) {
                    if (sort_cli_dm[j].first >= 0)
                        lower_avg_loc++;
                    else
                        break;
                }

                for (int idx = 0; idx < lower_avg_loc; idx++) {
                    int cli_idx = sort_cli_dm[idx].second;
                    // for(auto cli_idx: clis) {
                    int& cli_dm = row[cli_idx];
                    int& edge_rest_sb = sb_map_ref[edges_names[edge_idx]];
                    int dist_v = edge_rest_sb;
                ...
                int dist_edge = 0;
                if (lower_avg_loc < sort_cli_dm.size()) {
                    int edge_rest_sb = sb_map_ref[edges_names[edge_idx]];
                    dist_edge = edge_rest_sb / (sort_cli_dm.size() - lower_avg_loc);
                }
                while (true) {
                    int cli_dm_total = 0;
                    int& edge_rest_sb = sb_map_ref[edges_names[edge_idx]];
                    for (int idx = lower_avg_loc; idx < sort_cli_dm.size(); idx++) {
                        int cli_idx = sort_cli_dm[idx].second;
                        // for(auto cli_idx: clis) {
                        int& cli_dm = row[cli_idx];
                        int dist_v = dist_edge;
                        dist_v = std::min(dist_v, cli_dm);
                        cli_dm -= dist_v;
                        edge_rest_sb -= dist_v;
                        dist_v_acc += dist_v;
                    ...
                    if (cli_dm_total == 0 || edge_rest_sb == 0) break;
                }
    ...
    for (int i = 0; i < client_names.size(); i++) {
        clients_avg[i] /= data_dm_rowstore.size();
    }
...
void contest_calculate_95_with_maxrequire_sort20(
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
    std::unordered_map<std::string, int>& sb_map, int qos_constrain,
    ...
    std::vector<bool> first_alloc(edges_names.size());
    {
        ...
        for (int i = 0; i < edges_names.size(); i++) {
            ...
            while (five_count <= five_percent_time && require_idx < sort_require.size()) {
                ...
                for (int j = 0; j < clis.size(); j++) {
                    double cli_dm_diff =
                        (row[clis[j]] - clients_avg[clis[j]]) / clients_avg[clis[j]];
                    sort_cli_dm[j] = {cli_dm_diff, clis[j]};
                    // int cli_edge_nums = qos_map[clis[j]].size();
                    // sort_cli_dm[j] = {cli_edge_nums, clis[j]};
                }
    ...
    for (int i = 0; i < edges_names.size(); i++) {
        ...
        while (five_count <= five_percent_time && require_idx < sort_require.size()) {
            ...
            std::vector<std::pair<double, int>> sort_cli_dm(clis.size());
            for (int j = 0; j < clis.size(); j++) {
                double cli_dm_diff = (row[clis[j]] - clients_avg[clis[j]]) / clients_avg[clis[j]];
                sort_cli_dm[j] = {cli_dm_diff, clis[j]};
                // int cli_edge_nums = qos_map[clis[j]].size();
                // sort_cli_dm[j] = {cli_edge_nums, clis[j]};
            }
...
void contest_calculate_95_with_maxrequire_sort21(
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
    std::unordered_map<std::string, int>& sb_map, int qos_constrain,
...
void contest_calculate_95_with_maxrequire_sort20_adjust(
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
    std::unordered_map<std::string, int>& sb_map, int qos_constrain,
...
void contest_calculate_95_with_maxrequire_sort19_adjust(
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
    std::unordered_map<std::string, int>& sb_map, int qos_constrain,
    ...
    std::vector<bool> first_alloc(edges_names.size());
    {
        ...
        for (int i = 0; i < edges_names.size(); i++) {
            ...
            while (five_count <= five_percent_time && require_idx < sort_require.size()) {
                ...
                if (lower_avg_loc < sort_cli_dm.size()) {
                    int edge_rest_sb = sb_map_ref[edges_names[edge_idx]];
                    dist_edge = edge_rest_sb / (sort_cli_dm.size() - lower_avg_loc) + 1;
                }
                while (true) {
                    int cli_dm_total = 0;
                    int& edge_rest_sb = sb_map_ref[edges_names[edge_idx]];
                    for (int idx = lower_avg_loc; idx < sort_cli_dm.size() && edge_rest_sb; idx++) {
                        int cli_idx = sort_cli_dm[idx].second;
                        // for(auto cli_idx: clis) {
                        int& cli_dm = row[cli_idx];
                        int dist_v = dist_edge;
...
void choose_best_result2(
    std::vector<void (*)(std::vector<std::string>&, std::vector<std::string>&,
                         std::vector<std::vector<int>>&, std::vector<std::vector<int>>&,
                         std::vector<int>&, std::unordered_map<std::string, int>&, int,
                         std::vector<std::vector<std::vector<int>>>&)>& func_vector,
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
...
void adjust_best_result(
    std::vector<void (*)(std::vector<std::string>&, std::vector<std::string>&,
                         std::vector<std::vector<int>>&, std::vector<std::vector<int>>&,
                         std::vector<int>&, std::vector<std::vector<int>>,
                         std::unordered_map<std::string, int>&, int,
                         std::vector<std::vector<std::vector<int>>>&, double)>& func_vector,
    std::vector<std::string>& client_names, std::vector<std::string>& edges_names,
    std::vector<std::vector<int>>& data_dm_rowstore, std::vector<std::vector<int>>& qos_map,
    std::vector<int>& edge_dist_num, std::vector<std::vector<int>> edge_dist_clients,
    std::unordered_map<std::string, int>& sb_map, int qos_constrain,
    ...
    for (int i = 0; i < func_vector.size(); i++) {
        for (int j = 0; j < 200; j += 2) {
            ...
            auto sb_map_cp = sb_map;
            func_vector[i](client_names, edges_names, data_dm_rowstore_cp, qos_map, edge_dist_num,
                           edge_dist_clients, sb_map_cp, qos_constrain, res, thres);
            int score = calculate_94_score(res, client_names, edges_names);
            std::cout << "FUNC: " << i << " SCORE: " << score << std::endl;
            if (score < min_score) {
                min_score = score;
        ...
        //     auto data_dm_rowstore_cp = data_dm_rowstore;
        //     auto sb_map_cp = sb_map;
        //     func_vector[i](client_names, edges_names, data_dm_rowstore_cp, qos_map,
        //     edge_dist_num,
        //                 edge_dist_clients, sb_map_cp, qos_constrain, res, thres);
        //     int score = calculate_94_score(res, client_names, edges_names);
        //     // std::cout << "FUNC: " << i << " SCORE: " << score << std::endl;
    ...
    std::cout << "MIN FUNC: " << min_func << " MIN THRES: " << min_thres << " SCORE: " << min_score
              << std::endl;
    return;
}
...
int main() {
    ...
    std::vector<void (*)(std::vector<std::string>&, std::vector<std::string>&,
                         ...
                         std::vector<int>&, std::vector<std::vector<int>>,
                         std::unordered_map<std::string, int>&, int,
                         std::vector<std::vector<std::vector<int>>>&, double)>
        func_vector3 = {contest_calculate_95_with_maxrequire_sort19_adjust};
    adjust_best_result(func_vector3, client_names, edges_names, data_dm_rowstore, qos_map,
                       edge_dist_num, edge_dist_clients, sb_map, qos_constrain, res_for_print);
    // adjust_best_result(func_vector3, client_names, edges_names, data_dm_rowstore, qos_map,
    //                    edge_dist_num, edge_dist_clients, sb_map, qos_constrain, res_for_print);
    // res_redist1(res_for_print, client_names, edges_names, qos_map, edge_dist_clients, sb_map);
    // contest_calculate_95_with_maxrequire_sort21(client_names, edges_names, data_dm_rowstore,
    //                                             qos_map, edge_dist_num, edge_dist_clients,
    //                                             sb_map, qos_constrain, res_for_print);
